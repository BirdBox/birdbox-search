...
describe Birdbox::Search::Nest do
  ...
  it "must be able to paginate results" do
    ...
    results = Nest.fetch(members, tags, nil, :page => 1, :page_size => 5)
    results.count.must_equal(5)
    results = Nest.fetch(members, tags, nil, :page => 2, :page_size => 5)
    results.count.must_equal(1)
  end
  ...
  it "must be able to sort results" do
    members = { :facebook => ['100001', '100002'], :twitter => ['200001', '200002'] }
    tags = %w(california cheeseburger)
    ascending = Nest.fetch(members, tags, nil, :sort_by => :uploaded_at, :sort_direction => 'asc').map { |r| r.created_at }
    descending = Nest.fetch(members, tags, nil, :sort_by => :uploaded_at, :sort_direction => 'desc').map { |r| r.created_at }
  ...
  it "must support time-bounded queries" do
    members = { :facebook => ['100001', '100002'], :twitter => ['200001', '200002'] }
    tags = %w(california cheeseburger)
    results = Nest.fetch(members, tags, nil, :since => Time.parse("2013-01-01 00:00:00").to_i)
    results.count.must_equal(4)
    results = Nest.fetch(members, tags, nil, :until => Time.parse("2012-12-31 23:59:59").to_i)
    results.count.must_equal(2)
    results = Nest.fetch(members, tags, nil, :since => Time.parse("2012-12-01 00:00:00").to_i, :until => Time.parse("2012-12-31 23:59:59").to_i)
    results.count.must_equal(1)
  end
